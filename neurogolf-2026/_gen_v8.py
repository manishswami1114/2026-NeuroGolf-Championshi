"""Generate neurogolf-v8.ipynb.

v8 = v7 with ALGORITHMIC SOLVER EXPANSION.
...
def model_scale_up(k, ih, iw):
    '''Each input cell becomes a k x k block: output[r,c] = input[r//k, c//k].
    Output shape (ih*k, iw*k).'''
    oh_out, ow_out = ih*k, iw*k
    idx = np.zeros(H*W, np.int64)
    for r in range(H):
        for c in range(W):
            if r < oh_out and c < ow_out:
                sr, sc = r // k, c // k
                if sr < ih and sc < iw:
                    idx[r*W + c] = sr*W + sc
    return _gather_mask_model(idx, oh_out, ow_out)
...
def model_repeat_rows(k, ih, iw):
    '''Each row repeated k times: out[r*k:(r+1)*k, :] = in[r, :] broadcast.'''
    oh_out, ow_out = ih*k, iw
    idx = np.zeros(H*W, np.int64)
    for r in range(H):
        for c in range(W):
            if r < oh_out and c < ow_out:
...
out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'neurogolf-v8.ipynb')
