"""Generate neurogolf-v6.ipynb.

v6 = v5 with FIXED MLP solver (v5 regressed to 201 pts; v6 targets 500+).

Diagnosis of v5's 0 MLP solves:
  * MSE loss with one-hot sparse targets (mean=0.1) gets stuck near all-zeros
    -- `(out>0)==Y` accuracy stays at 0/N even as loss looks small.
  * hidden=[4,8,16,32,64] starts too narrow; most tasks need 16-128.
  * per_hidden_budget = timeout/(H-1) starved the wider arches.

Key fixes (measured on 29-task diagnostic sample: 0 -> 10 MLP solves):
  * **BCE with scaled logits**: `F.binary_cross_entropy_with_logits(out*2, Y)`.
    Handles sparse targets; separates signs cleanly around 0 (matches the
    competition's `>0 -> 1` threshold).
  * **Wider sweep**: hidden=[8,16,32,64,128,256], smallest-first for best score.
  * **Shape filter**: drop training pairs whose (in,out) shape is not the
    dominant one (rare outlier arc-gen pairs poison training).
  * **Simple time budget**: 30s total per task; most solves happen in < 2s.
  * Keeps every v4/v5 handcrafted detector + Gather-based linear solver + CNN tier.
"""
import json, os
...
md("""# NeuroGolf 2026 -- Competitive Solver v6

**Jump from v5 (201 pts -> target 500+):**
- **FIXED MLP solver**: v5 got 0 MLP solves due to MSE loss on one-hot
  targets (stuck near zeros).  v6 uses `BCE(out*2, Y)` + wider hidden =
  ~10/29 diagnostic solves (extrapolates to ~100-140 tasks).
- Shape-filter: drops outlier arc-gen pairs so MLP trains clean.
- Keeps every v4 handcrafted detector + linear solver + CNN tier.
""")
...
def try_mlp_solve(all_pairs, timeout=MLP_TIMEOUT):
    ...
    Must validate on the ORIGINAL all_pairs via run_onnx, so the caller's
    check_all() still has the final say -- this function just returns a
    candidate model that passed its own training-set accuracy check.

    Key: uses BCE with scaled logits (`out*2`).  MSE fails silently on
    sparse one-hot targets (stuck near zeros), BCE does not.'''
    ...
    X = torch.tensor(np.stack([g2small(p['input'],  ih,  iw ) for p in pairs]), dtype=torch.float32).to(DEVICE)
    Y = torch.tensor(np.stack([g2small(p['output'], oh_, ow_) for p in pairs]), dtype=torch.float32).to(DEVICE)
    N = len(pairs)

    # Smallest-first for best score.  hidden=4 is too narrow; skip.
    candidate_hidden = [8, 16, 32, 64, 128, 256]
    t0 = time.time()
    best_arch = None  # (hidden, state_dict)

    for hidden in candidate_hidden:
        if time.time() - t0 > timeout: break
        n_params = in_dim*hidden + hidden + hidden*out_dim + out_dim
        # Reserve ~50KB for ONNX graph overhead
        ...
        for epoch in range(5000):
            ...
            opt.zero_grad()
            out = model(X)
            # BCE on scaled logits: MUCH better than MSE for one-hot Y.
            loss = F.binary_cross_entropy_with_logits(out*2, Y)
            loss.backward()
            opt.step()
            if (epoch+1) % 25 == 0:
                model.eval()
                with torch.no_grad():
                    ...
                    correct = int(torch.all(pred == Y, dim=1).sum().item())
                    if correct == N:
                        solved = True
        ...
        if solved: break  # Keep the smallest hidden that works
...
def try_mlp_fixed_out_solve(all_pairs, timeout=MLP_TIMEOUT):
    ...
    output.  Works when output shape is constant across pairs but input varies
    (e.g. "summarize any-size grid into a 3x3").'''
    out_shapes = set()
    for p in all_pairs:
    ...
    # Input: full 30x30 padded tensor (same encoding used by the scorer)
    X = torch.tensor(np.stack([g2t(p['input']).flatten() for p in all_pairs]),
                     dtype=torch.float32).to(DEVICE)
    # Output: small fixed-shape target
    def g2small(grid, h, w):
        t = np.zeros((C, h, w), np.float32)
        for r in range(h):
            for c in range(w):
                if r < len(grid) and c < len(grid[r]):
                    ...
                    if 0 <= v <= 9: t[v, r, c] = 1.0
        return t.flatten()
    Y = torch.tensor(np.stack([g2small(p['output'], oh_, ow_) for p in all_pairs]),
                     dtype=torch.float32).to(DEVICE)

    candidate_hidden = [4, 8, 16, 32, 64]
    t0 = time.time()
    best_arch = None
    for hidden in candidate_hidden:
        if time.time() - t0 > timeout: break
...
out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'neurogolf-v6.ipynb')
